<template>
  <div class="autocomplete">
    ...
    <ul v-show="showResults" class="autocomplete__results" :style="listStyle">
      <slot name="results">
        <!-- error -->
        <li v-if="hasError" class="autocomplete__results__item autocomplete__results__item--error">{{ error }}
        </li>

        <!-- results -->
        <template v-if="!hasError">
          <slot name="firstResult" />
          <li
  ...
  methods: {
    ...
    getHeaders() {
      ...
      if (this.requestHeaders) {
        for (const prop in this.requestHeaders) {
