  ...
  <div class="app-container">
      ...
      <v-container fluid>
        <v-row>
          ...
          <v-col md="3" sm="5" class="search-right-wrap">
            ...
            <el-button type="primary" plain class="btn-search" @click="getRobotVersion">
    ...
    <v-card class="table-users">
      ...
      >
        ...
        <el-table-column :label="generateTitleView('name', 'robotVersion')" min-width="120">
          <template slot-scope="{row}">
            <span class="link-type">{{ row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="generateTitleView('description', 'robotVersion')" align="center">
          <template slot-scope="{row}">
            ...
            <div>
              ...
              >
                <el-toggle-button
                  ...
                  color="#123D8B"
                  :sync="true"
                  @change="updateRobotVersionStatus(row.id, $event)"
                />
              </el-tooltip>
            </div>
          </template>
        ...
        >
          <template slot-scope="{row}">
            <el-tooltip
              class="item"
              effect="dark"
              :content="generateTitleView('updateTooltip', 'robotVersion')"
              placement="top"
            >
              <el-button type="primary" size="mini" rounded @click="editRobotVersion(row.id)">
            ...
            <el-tooltip
              class="item"
              effect="dark"
              :content="generateTitleView('deleteTooltip', 'robotVersion')"
              placement="top"
            >
              <el-button size="mini" type="danger" @click="deleteRobotVersion(row.id)">
      ...
      <pagination
        v-show="totalRobotVersion>0"
        class="pagination-hotel"
        :total="totalRobotVersion"
        :page.sync="listQuery.page"
        :limit.sync="listQuery.limit"
        @pagination="getRobotVersion"
      />
    </v-card>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      language: 'language',
      listRobotVersion: LABEL.model + 'listRobotVersion',
      totalRobotVersion: LABEL.model + 'totalRobotVersion'
    })
  },
  mounted() {
  ...
  methods: {
    getRobotVersion() {
      this.$store.dispatch(LABEL.model + ACTIONS.ACT_GET_ROBOT_VERSION_PAGING, {
    ...
    deleteRobotVersion(userID) {
      const permissionAction = this.hasPermissionAction(this.roleList, Permissions.RobotVersionDelete, '', this.generateTitleView('permissionMessage', 'message'))
      if (permissionAction) {
        this.$confirm(this.generateTitleView('textMessageDelete', 'message'), this.generateTitleView('titleMessageDelete', 'message'), {
          ...
          this.$store.dispatch(LABEL.model + ACTIONS.ACT_DELETE_ROBOT_VERSION, { userID })
        })
      }
    },
    editRobotVersion(id) {
      this.hasPermissionAction(this.roleList, Permissions.RobotVersionEdit, 'users/robot-version/edit/' + id, this.generateTitleView('permissionMessage', 'message'))
    },
    resetFilter() {
    ...
    updateRobotVersionStatus(userId, event) {
      const permissionAction = this.hasPermissionAction(this.roleList, Permissions.RobotVersionEdit, '', this.generateTitleView('permissionMessage', 'message'))
      if (permissionAction) {
        var dataUpdate = {
        ...
        this.$store
            .dispatch(LABEL.model + ACTIONS.ACT_UPDATE_ROBOT_VERSION_STATUS, dataUpdate)
            .then(res => {
              if (res.isSuccess) {
                this.$notify.success({
    ...
    redirectCreate() {
      this.hasPermissionAction(this.roleList, Permissions.RobotVersionDelete, 'users/robot-version/create', this.generateTitleView('permissionMessage', 'message'))
    },
    generateTitleView,
    hasPermissionAction
  }
